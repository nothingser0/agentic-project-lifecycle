...
| Gate 2 | Q0f | Charter approval state is explicit; kick-off can be scheduled safely; **`docs/pm/STAKEHOLDERS.md` exists with governance roles identified** (Sponsor, Budget Approver, Decision Owner); solo projects: `docs/pm/STAKEHOLDERS.md` paragraph written instead | Keep charter draft/review status visible; block kick-off if governance authority is undefined |
...
| Gate T | Phase 2a | Schedule model, milestones, dependencies, baseline and deviation rules exist; **`docs/pm/RACI.md` exists and has no empty role columns for active operational delivery roles** (Medium+ — per Cross-phase rule 21; for lean teams without dedicated specialists, roles collapse to Primary Sponsor per `02a-planning-core-phases.md`); solo projects: `docs/pm/STAKEHOLDERS.md` paragraph written instead | Stay in timeline phase; no “on track” claim or Phase 3 development entry without both schedule and operational ownership baselines |
...
5. **Gate D (development start) for Medium+ projects requires an operational test harness and verified baseline command.** `VERIFY.md` must be configured with runnable test/lint/typecheck commands that pass (exit code 0 on initial smoke test). The **80%+ overall test coverage threshold (and 100% on payment/auth paths)** is enforced during the Build phase at `gate:pr`, `gate:merge`, and before production deploy — not on unwritten code prior to development start. See `SKILL.md` Cross-phase rule 9 and `references/qa/TESTING_STRATEGY_DETAIL.md`.
