...
21. **Conflict resolution is systematic.** Medium+ projects define escalation ladder (Direct → Mediation → Authority → Steering → Legal), RACI matrix with veto power, prioritization framework (RICE/WSJF), decisions documented in ADR. See `references/pm/CONFLICT_RESOLUTION_GUIDE.md`. Operational RACI matrix generated from `templates/pm/RACI_MATRIX_TEMPLATE.md` → `docs/pm/RACI.md` in Phase 2a after team allocation is baselined. Mandatory at Gate T (gate:timeline-complete). Solo projects: replace RACI with one paragraph in `docs/pm/STAKEHOLDERS.md`.
