...
## 5. Vector Database & Embedding Model Lifecycle

Embedding vectors are mathematical projections tied strictly to the generating model's latent space and dimensions. You cannot compare or query vectors generated by different models or different dimensions.

### Rule 1: Metadata Tagging on Every Vector Row
Every vector stored in pgvector, Pinecone, Qdrant, or Milvus MUST store embedding provenance:

```sql
ALTER TABLE document_embeddings ADD COLUMN embedding_model TEXT NOT NULL DEFAULT 'text-embedding-3-small';
ALTER TABLE document_embeddings ADD COLUMN chunk_strategy TEXT NOT NULL DEFAULT 'recursive_500_overlap_50';
ALTER TABLE document_embeddings ADD COLUMN embedded_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
```

### Rule 2: Zero-Downtime Embedding Model Migration
When upgrading embedding models (e.g. `text-embedding-ada-002` → `text-embedding-3-small`):

1. **Never update vectors in place.** Modifying vectors row-by-row creates a corrupted hybrid state where semantic search fails unpredictably.
2. **Create a parallel target collection:** `documents_v2` with the new vector dimensionality.
3. **Dual-write or background backfill:** Worker reads original text chunks from source storage, embeds with new model, and populates `documents_v2`.
4. **Retrieval evaluation gate:** Run the golden evaluation benchmark (`tests/evals/eval-runner.test.ts`) asserting that retrieval Recall@K and NDCG on `documents_v2` is equal to or better than `documents_v1`.
5. **Atomic routing swap:** Update environment variable `VECTOR_COLLECTION_NAME=documents_v2`.
6. **Grace period & teardown:** Retain `documents_v1` for 7 days for instant rollback capability, then drop.
